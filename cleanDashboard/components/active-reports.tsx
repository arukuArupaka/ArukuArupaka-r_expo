...
export function ActiveReports({ reports, onBack }: ActiveReportsProps) {
  return (
    <div>
      ...
      <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
        <div className="flex items-center gap-2 text-green-800">
          ...
          <span className="font-medium">現在 {reports.length} 件の案件が進行中です</span>
        </div>
      </div>
      ...
      {reports.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">進行中の案件はありません</h3>
          <p className="text-gray-600">現在進行中の案件がありません。</p>
        </div>
      )}
    </div>
  )
