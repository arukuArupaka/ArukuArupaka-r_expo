...
import {
  setCampusDataAction,
  setMapUserObject,
  setcampusBuildingDataAction,
} from "../../redux/actions/mapUserActions";
...
const MapMainView = () => {
  ...
  const uploadImageAsync = async (
    ...
    const storageRef = ref(
      storage,
      `mapBuildings/${campusData.id}/${ID}/mainBuildingImage`
    );
  ...
  useEffect(() => {
    ...
    const getCampusBuildingData = async () => {
      ...
      await getDoc(refFiresrore)
        ...
        .catch((error) => {
          console.log(error);
          dispatch(setcampusBuildingDataAction([]));
        });
    };
    if (campusData.id) {
    ...
    <View style={{ position: "relative" }}>
        ...
        >
          ...
          <ScrollView style={{ paddingHorizontal: 20, marginTop: 20 }}>
            {CampusLocationData.map((campusData, index) => (
              <TouchableOpacity
                onPress={() => selectCampus(campusData)}
                style={{
                  height: 60,
                  borderBottomWidth: 0,
                  alignItems: "center",
              ...
              >
                <Image
                  ...
                  source={{ uri: campusData.imageURL }}
                />
                <Text style={{}}>{campusData.name}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
      ...
      {!showCampusSelect && (
        <View>
          {showMap ? (
            <DisplayList
              campusBuildingsArray={campusBuildingsArray}
              openMap={() => toggleComponent()}
            ...
            <DisplayMap
              ...
              isEditBuilding={showEditBuilding}
              onPickLongitudeLatitude={(event) => {
                pickBuildinglocation(event);
              }}
            />
          )}
        </View>
      ...
      >
        <Ionicons
          name={showMap ? "map-outline" : "list"}
          size={30}
          color="#EB3637"
        />
      </TouchableOpacity>
      {Object.keys(buildingLocation).length !== 0 && showEditBuilding && (
        <View
          style={{
            position: "absolute",
            width: "100%",
