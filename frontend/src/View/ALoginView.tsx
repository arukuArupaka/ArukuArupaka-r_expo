...
const ALoginView = (props) => {
  ...
  const [showResendRegisterBotton, setShowResendRegisterBotton] =
    useState(false);
  ...
  const loginAfterPageName = useSelector(
    (state) => state.common.loginAfterPageName
  );
  ...
  const handleLogin = async () => {
    try {
      ...
      if (await isMailVerified()) {
        ...
        await saveCredentials(email, password); // Save credentials upon successful login
        console.log("ここにだす");
  ...
  const completeCreateAccount = async () => {
    try {
      ...
      console.log(error);
      setErrorMessage(
        "申し訳ありません。予期しないエラーが発生しました。時間を空けて再度行ってください。"
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Ionicons
          name="arrow-back"
          style={{ marginVertical: 6, color: "white", textAlign: "center" }}
          size={24}
          color="black"
        />
      </TouchableOpacity>
      ...
      >
        ...
        <Image
          ...
          source={require("../image/logo/icon-1024.png")}
        ></Image>
      </View>
      <View
        style={{
      ...
      <View
        style={{ flexDirection: "row", marginBottom: 20, marginHorizontal: 30 }}
      >
        {isCreateAcount && (
          <TouchableOpacity style={{ flex: 1 }} onPress={switchLogin}>
            <Text style={{ textAlign: "center", color: "#C8252B" }}>
              ログインはこちら
            </Text>
          </TouchableOpacity>
        )}
        {(isLogin || isResetPass) && (
          <TouchableOpacity style={{ flex: 1 }} onPress={switchCreateAccount}>
            <Text style={{ textAlign: "center", color: "#C8252B" }}>
              アカウント作成
            </Text>
          </TouchableOpacity>
        ...
        {(isLogin || isResetPass) && (
          <TouchableOpacity style={{ flex: 1 }} onPress={switchResetPassWord}>
            <Text style={{ textAlign: "center", color: "#C8252B" }}>
              パスワードのリセット
            </Text>
          </TouchableOpacity>
      ...
      {showCreateAccount && isCreateAcount && (
        <KeyboardAvoidingView style={{ width: "80%" }} behavior="padding">
          ...
          {showResendRegisterBotton && (
            <TouchableOpacity
              onPress={setdRegisterMail}
