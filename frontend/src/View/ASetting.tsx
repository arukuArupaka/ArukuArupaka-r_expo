  ...
  const sendUserDate = async () => {
    if (isLogin) {
      ...
      if (
        ...
        JSON.stringify(Object.entries(oldDate).sort())
      ) {
    ...
    <ScrollView contentInsetAdjustmentBehavior="automatic">
      <SafeAreaView>
        ...
        <View style={{ marginHorizontal: 10, marginTop: 40 }}>
          ...
          >
            <Text
              style={{ textAlign: "center", fontSize: 20, color: "#C8252B" }}
            >
              アカウントを削除
            </Text>
          </TouchableOpacity>
          {isShowDelateConfig && (
            <Dialog.Container visible={true}>
              ...
              <Dialog.Input
                onChangeText={(text) => setDelateConfigWord(text)}
              />
              <View>
                <Dialog.Button
                  label="アカウント削除"
