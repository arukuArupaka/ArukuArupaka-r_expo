...
export const TimeTableProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://render-test-4ya9.onrender.com//time_table/get/all/search/?kamoku_department=${department}&kamoku_day=${day}&kamoku_time=${time}&kamoku_season=${season}`);
        if (!response.ok) {
